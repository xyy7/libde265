...
void decoded_picture_buffer::log_dpb_content() const
{
  for (size_t i = 0; i < dpb.size(); i++)
  {
    loginfo(LogHighlevel, " DPB %d: POC=%d, ID=%d %s %s\n", i,
            ...
            dpb[i]->get_ID(),
            dpb[i]->PicState == UnusedForReference ? "unused" : dpb[i]->PicState == UsedForShortTermReference ? "short-term"
                                                                                                              : "long-term",
                                      ...
                                      de265_PTS pts, void *user_data, bool isOutputImage)
{
  ...
  {
  case 0:
    chroma = de265_chroma_mono;
    break;
  case 1:
    chroma = de265_chroma_420;
    break;
  case 2:
    chroma = de265_chroma_422;
    break;
  case 3:
    chroma = de265_chroma_444;
    break;
  default:
    chroma = de265_chroma_420;
    assert(0);
