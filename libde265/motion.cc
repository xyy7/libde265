             ...
             int nPbW, int nPbH, int bitDepth_L)
{
  ...
  ALIGNED_16(int16_t)
  mcbuffer[MAX_CU_SIZE * (MAX_CU_SIZE + 7)];
                                       ...
                                       const PBMotion *vi)
{
  ...
  ALIGNED_16(int16_t)
  predSamplesL[2 /* LX */][MAX_CU_SIZE * MAX_CU_SIZE];
  ALIGNED_16(int16_t)
  predSamplesC[2 /* chroma */][2 /* LX */][MAX_CU_SIZE * MAX_CU_SIZE];
...
bool PBMotion::operator==(const PBMotion &b) const
{
  ...
  for (int i = 0; i < 2; i++)
  {
    if (a.predFlag[i] != b.predFlag[i])
      return false;

    if (a.predFlag[i])
    {
      if (a.mv[i].x != b.mv[i].x)
        return false;
      if (a.mv[i].y != b.mv[i].y)
        return false;
      if (a.refIdx[i] != b.refIdx[i])
        return false;
    }
  }
    ...
    int maxCandidates)
{
  ...
  */
  ...
  if (computed_candidates >= maxCandidates)
  ...
  else
  {
    ...
    if (!availableB1)
      logtrace(LogMotion, "spatial merging candidate B1: unavailable\n");
  }
  ...
  else
  {
    ...
    if (!availableA0)
      logtrace(LogMotion, "spatial merging candidate A0: unavailable\n");
  }
  ...
  else
  {
    ...
    if (!availableB2)
      logtrace(LogMotion, "spatial merging candidate B2: unavailable\n");
  }
