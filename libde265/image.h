...
public:
  ...
  {
    if (data)
      memset(data, 0, sizeof(DataUnit) * data_size);
  }
...
struct CTB_info
{
  ...
  friend std::ostream &operator<<(std::ostream &os, const CTB_info &obj)
  {
    os << "SliceAddrRS: " << obj.SliceAddrRS << ", SliceHeaderIndex: " << obj.SliceHeaderIndex << std::endl;
    os << "saoInfo: " << obj.saoInfo << std::endl;
    os << "deblock: " << (obj.deblock ? "true" : "false") << std::endl;
    os << "has_pcm_or_cu_transquant_bypass: " << (obj.has_pcm_or_cu_transquant_bypass ? "true" : "false");
    return os;
  }
};
...
struct CB_ref_info
{
  ...
  friend std::ostream &operator<<(std::ostream &os, const CB_ref_info &info)
  {
    os << "log2CbSize: " << static_cast<int>(info.log2CbSize)
       << ", PartMode: " << static_cast<int>(info.PartMode)
       << ", ctDepth: " << static_cast<int>(info.ctDepth)
       << ", PredMode: " << static_cast<int>(info.PredMode)
       << ", pcm_flag: " << static_cast<int>(info.pcm_flag)
       << ", cu_transquant_bypass: " << static_cast<int>(info.cu_transquant_bypass)
       << ", QP_Y: " << static_cast<int>(info.QP_Y);
    return os;
  }
};
...
struct de265_image
{
  ...
  de265_image& operator=(const de265_image& other) {
        ...
        if (this != &other) {
          ...
          stride = other.stride;
          chroma_stride = other.chroma_stride;
          ...
          for (int i = 0; i < 3; ++i)
          {
            ...
            if (other.chroma_format == CHROMA_MONO){
              break;
            }
          }
          for (int i = 0; i < 3; ++i)
