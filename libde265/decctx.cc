...
de265_error decoder_context::decode_some(bool *did_work)
{
  ...
  *did_work = false;
  ...
  if (!image_units.empty())
  { // && ! image_units[0]->slice_units.empty() ) {
                                                   ...
                                                   slice_unit *sliceunit)
{
  ...
  for (int entryPt = 0; entryPt < nRows; entryPt++)
  {
    ...
    {
      dataStartIndex = shdr->entry_point_offset[entryPt - 1];
    }

    int dataEnd;
    if (entryPt == nRows - 1)
      dataEnd = sliceunit->reader.bytes_remaining;
    else
      dataEnd = shdr->entry_point_offset[entryPt];
  ...
  else
  {
         ...
         i++)
    {
      ...
      if (hdr->delta_poc_msb_present_flag[i])
      {
        ...
        pocLt += currentPictureMSB - DeltaPocMsbCycleLt[i] * current_sps->MaxPicOrderCntLsb;
      }
...
int decoder_context::get_highest_TID() const
{
  if (current_sps)
  {
    return current_sps->sps_max_sub_layers - 1;
  }
  if (current_vps)
  {
    return current_vps->vps_max_sub_layers - 1;
  }
